...
def par_matrix_multiply(A, B):
    ...
    # if num_rows_A * num_cols_B < 900_000:
...
if __name__ == '__main__':
    NUM_EVAL_RUNS = 1
    num = 600  # when num is small, sequential would be faster
    A = [[random.random() for i in range(num)] for j in range(num)]
    B = [[random.random() for i in range(num)] for j in range(num)]
    ...
    print('Efficiency: {:.2f}%'.format(100 * (sequential_time / parallel_time) /
                                       mp.cpu_count()))
